...
export function useNetworkStatus(options: NetworkStatusOptions = {}) {
  ...
  const updateStatus = useCallback(() => {
    ...
    setState(prev => {
      ...
      if (prev.isSlowConnection !== newState.isSlowConnection && onSlowConnection) {
  ...
  return {
    ...
    connectionQuality: state.isSlowConnection ? 'poor' :
                      state.effectiveType === '4g' ? 'excellent' :
                      state.effectiveType === '3g' ? 'good' : 'fair'
  };
}