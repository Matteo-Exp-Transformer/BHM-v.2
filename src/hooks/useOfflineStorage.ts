...
class OfflineStorageManager {
  ...
  private setupStores(db: IDBDatabase): void {
    ...
    if (!db.objectStoreNames.contains(STORES.CONSERVATION_POINTS)) {
      const store = db.createObjectStore(STORES.CONSERVATION_POINTS, {
        keyPath: 'id',
      })
    ...
    if (!db.objectStoreNames.contains(STORES.TEMPERATURE_READINGS)) {
      const store = db.createObjectStore(STORES.TEMPERATURE_READINGS, {
        keyPath: 'id',
      })
    ...
    if (!db.objectStoreNames.contains(STORES.MAINTENANCE_TASKS)) {
      const store = db.createObjectStore(STORES.MAINTENANCE_TASKS, {
        keyPath: 'id',
      })
      store.createIndex('conservation_point_id', 'conservation_point_id')
    ...
    if (!db.objectStoreNames.contains(STORES.CALENDAR_EVENTS)) {
      const store = db.createObjectStore(STORES.CALENDAR_EVENTS, {
        keyPath: 'id',
      })
  ...
  async getByIndex<T>(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(
          new Error(`Failed to get indexed data: ${request.error?.message}`)
        )
    })
  }
...
export function useOfflineStorage() {
  ...
  const get = useCallback(
    async <T>(storeName: StoreName, key: string): Promise<T | null> => {
      try {
  ...
  const getAll = useCallback(
    async <T>(storeName: StoreName, limit?: number): Promise<T[]> => {
      try {
  ...
  const getByIndex = useCallback(
    async <T>(
      ...
      try {
        return await storageManager.getByIndex<T>(
          storeName,
          indexName,
          value,
          limit
