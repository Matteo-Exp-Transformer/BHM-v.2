...
export function useCalendar(options: UseCalendarOptions = {}) {
  ...
  const transformToCalendarEvents = useCallback((data: CalendarData): TypedCalendarEvent[] => {
    ...
    if (filter.sources.includes('inventory')) {
      data.inventoryEvents.forEach(invEvent => {
        const event: InventoryEvent = {
          ...
          extendedProps: {
            description: invEvent.description || `Evento di inventario: ${invEvent.type}`,
    ...
    return events.filter(event => {
      return filter.priorities.includes(event.extendedProps.priority) &&
        ...
        case 'maintenance':
          const { data: maintenanceData, error: maintenanceError } = await supabase
            ...
            .insert({
              ...
              estimated_duration: eventData.extendedProps?.estimatedDuration || 30,
              conservation_point_id: eventData.extendedProps?.conservationPointId,
  ...
  const updateEventMutation = useMutation({
    mutationFn: async ({ eventId, updates }: { eventId: string; updates: Partial<TypedCalendarEvent> }) => {
...
function determineMaintenancePriority(task: any): CalendarEvent['extendedProps']['priority'] {
  const now = new Date();
...
function determineMaintenanceStatus(task: any): CalendarEvent['extendedProps']['status'] {
  const now = new Date();
  const dueDate = new Date(task.next_due_date);
