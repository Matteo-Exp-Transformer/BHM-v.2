...
export interface MaintenanceEvent extends Omit<CalendarEvent, 'source' | 'extendedProps'> {
  source: 'maintenance';
...
export interface TaskEvent extends Omit<CalendarEvent, 'source' | 'extendedProps'> {
  source: 'task';
  extendedProps: CalendarEvent['extendedProps'] & {
    taskType: 'haccp_check' | 'inventory_count' | 'quality_control' | 'admin' | 'other';
    departmentId?: string;
...
export interface TrainingEvent extends Omit<CalendarEvent, 'source' | 'extendedProps'> {
  source: 'training';
  extendedProps: CalendarEvent['extendedProps'] & {
...
export interface InventoryEvent extends Omit<CalendarEvent, 'source' | 'extendedProps'> {
  source: 'inventory';
  extendedProps: CalendarEvent['extendedProps'] & {
    inventoryType: 'expiry_check' | 'stock_count' | 'delivery' | 'order' | 'waste_disposal';
    productIds?: string[];
    supplierId?: string;
...
export interface MeetingEvent extends Omit<CalendarEvent, 'source' | 'extendedProps'> {
  source: 'meeting';
  extendedProps: CalendarEvent['extendedProps'] & {
...
export type TypedCalendarEvent = MaintenanceEvent | TaskEvent | TrainingEvent | InventoryEvent | MeetingEvent;
