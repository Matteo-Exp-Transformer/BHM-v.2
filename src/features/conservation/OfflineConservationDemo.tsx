...
export function OfflineConservationDemo() {
  ...
  const [selectedPoint, setSelectedPoint] = useState<ConservationPoint | null>(null);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Sistema Conservazione - Demo Offline</h1>
            <p className="text-gray-600">Test del sistema di sincronizzazione offline</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
            ...
            <div>
              ...
              <div className="text-lg font-bold text-blue-700">{pendingOperations.length}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-red-50 rounded-lg">
            ...
            <div>
              ...
              <div className="text-lg font-bold text-red-700">{syncErrors.length}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-green-50 rounded-lg">
            ...
            <div>
              <div className="text-sm font-medium text-green-900">Connessione</div>
              <div className="text-sm font-medium text-green-700 capitalize">{connectionQuality}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
            ...
            <div>
              <div className="text-sm font-medium text-gray-900">Ultimo Sync</div>
              <div className="text-sm text-gray-700">
                {lastSyncTime ? lastSyncTime.toLocaleTimeString('it-IT') : 'Mai'}
              </div>
            </div>
          </div>
        </div>
        ...
        {hasPendingOperations && (
          <div className="mt-4 p-3 bg-yellow-50 rounded-lg">
            <h4 className="text-sm font-medium text-yellow-800 mb-2">Operazioni in Attesa</h4>
            <div className="space-y-1">
              {pendingOperations.slice(0, 3).map((op) => (
                <div key={op.id} className="flex justify-between text-sm text-yellow-700">
                  <span>{op.type} {op.entity}</span>
                  <span>{new Date(op.timestamp).toLocaleTimeString('it-IT')}</span>
                </div>
              ))}
        ...
        {hasErrors && (
          <div className="mt-4 p-3 bg-red-50 rounded-lg">
            <h4 className="text-sm font-medium text-red-800 mb-2">Errori di Sincronizzazione</h4>
            <div className="space-y-1">
              {syncErrors.slice(0, 2).map((error, index) => (
                <div key={index} className="text-sm text-red-700">
                  <div className="font-medium">{error.operation.type} {error.operation.entity}</div>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">Punti di Conservazione</h3>
          <p className="text-sm text-gray-600 mt-1">Clicca su un punto per aggiungere una lettura temperatura (modalità offline)</p>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-blue-900 mb-3">Istruzioni Demo</h3>
        <div className="space-y-2 text-sm text-blue-800">
          <p>• <strong>Modalità Online:</strong> Le letture vengono salvate direttamente su Supabase e anche in cache locale</p>
          <p>• <strong>Modalità Offline:</strong> Le letture vengono salvate localmente e messe in coda per la sincronizzazione</p>
          <p>• <strong>Test Offline:</strong> Disabilita la connessione per testare la modalità offline</p>
          <p>• <strong>Sincronizzazione:</strong> Al ritorno online, i dati vengono sincronizzati automaticamente</p>
        </div>
      </div>
