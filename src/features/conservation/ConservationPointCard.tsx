...
export function ConservationPointCard({
  ...
  const getTypeLabel = (type: ConservationPoint['type']) => {
    switch (type) {
      case 'freezer': return 'Congelatore';
  ...
  const pendingMaintenance = point.maintenance_tasks?.filter(
  ...
  return (
    <div className={`bg-white rounded-lg border-2 transition-colors hover:shadow-md ${getStatusColor(point.status)}`}>
      {/* Header */}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-lg font-bold ${
                latestReading.status === 'compliant' ? 'text-green-600' :
                latestReading.status === 'warning' ? 'text-yellow-600' : 'text-red-600'
              }`}>
            ...
            <div className="flex items-center justify-between text-xs text-gray-500">
              ...
              <span className={`px-2 py-0.5 rounded-full font-medium ${
                latestReading.status === 'compliant' ? 'bg-green-100 text-green-700' :
                latestReading.status === 'warning' ? 'bg-yellow-100 text-yellow-700' :
                'bg-red-100 text-red-700'
              }`}>
                {latestReading.status === 'compliant' ? 'Conforme' :
                 latestReading.status === 'warning' ? 'Attenzione' : 'Critico'}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1 text-xs text-gray-500">
              ...
              <span>Metodo: {latestReading.method === 'manual' ? 'Manuale' : 
                              latestReading.method === 'digital_thermometer' ? 'Termometro' : 'Sensore'}</span>
            </div>
          </div>
        ) : (
            ...
            {point.maintenance_tasks.slice(0, 2).map(task => (
              <div key={task.id} className="flex items-center justify-between text-xs">
                ...
                <span className={`font-medium ${
                  new Date(task.next_due_date) <= new Date() ? 'text-red-600' : 'text-gray-500'
                }`}>
