/**
 * 🔐 InviteAcceptForm - Componente Form Accettazione Invito
 * 
 * Nuovo componente form per accettare invito e creare account con
...
export const InviteAcceptForm: React.FC<InviteAcceptFormProps> = ({
  ...
  useEffect(() => {
    ...
    if (!token) {
      setErrors({ general: 'Token di invito mancante. Verifica il link ricevuto via email.' })
    ...
    const verifyInvite = async () => {
      try {
        const response = await authClient.verifyInvite(token)
        ...
        setErrors({ general: 'Errore durante la verifica dell\'invito.' })
      } finally {
        setIsVerifying(false)
      }
    }
  ...
  const handleInputChange = useCallback((field: keyof InviteAcceptData, value: string) => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // Validate form
    const validation = validateForm(inviteAcceptSchema, formData)
    ...
    try {
      ...
      if (response.success) {
        ...
        const errorMessage = response.error?.message || 'Errore durante la creazione dell\'account'
    ...
    return (
      <div className={`text-center ${className}`}>
        ...
        <p className="text-gray-600">
          Stiamo verificando la validità del tuo invito
        </p>
      </div>
    )
  }
  ...
  return (
    <form 
      ...
      noValidate
      aria-label="Form accettazione invito"
    >
      {/* Invite Info */}
      {inviteInfo && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
          ...
          <div className="text-xs text-green-600 space-y-1">
            ...
            <p><strong>Ruolo:</strong> {inviteInfo.role}</p>
            <p><strong>Organizzazione:</strong> {inviteInfo.tenant}</p>
          </div>
        </div>
      )}
      ...
      <div>
        ...
        <input
          ...
          placeholder="Mario"
          aria-invalid={hasFieldError(errors, 'firstName')}
          aria-describedby={hasFieldError(errors, 'firstName') ? 'invite-first-name-error' : undefined}
      ...
      <div>
        ...
        <input
          ...
          placeholder="Rossi"
          aria-invalid={hasFieldError(errors, 'lastName')}
          aria-describedby={hasFieldError(errors, 'lastName') ? 'invite-last-name-error' : undefined}
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            aria-describedby={hasFieldError(errors, 'password') ? 'invite-password-error' : undefined}
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            aria-describedby={hasFieldError(errors, 'confirmPassword') ? 'invite-confirm-password-error' : undefined}
