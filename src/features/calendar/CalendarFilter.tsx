...
export function CalendarFilter({ filter, onFilterChange, totalEvents }: CalendarFilterProps) {
  ...
  const clearAllFilters = () => {
    onFilterChange({
      ...
      priorities: Object.keys(priorityLabels) as Array<keyof typeof priorityLabels>,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Sources Filter */}
        <div>
          <h3 className="text-sm font-medium text-gray-900 mb-3">Tipologia Eventi</h3>
          <div className="space-y-2">
            {Object.entries(sourceLabels).map(([source, label]) => {
              ...
              return (
                <button
                  key={source}
                  onClick={() => toggleSource(source as keyof typeof sourceLabels)}
          ...
          <div className="space-y-2">
            {Object.entries(priorityLabels).map(([priority, label]) => {
              ...
              return (
                <button
                  key={priority}
                  onClick={() => togglePriority(priority as keyof typeof priorityLabels)}
                  className={`w-full flex items-center justify-between p-2 rounded-lg text-sm transition-colors ${
                    isSelected
