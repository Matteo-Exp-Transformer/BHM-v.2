...
export function Calendar() {
  ...
  const [selectedEvent, setSelectedEvent] = useState<TypedCalendarEvent | null>(null);
  ...
  const getStatusBorder = (status: string) => {
    switch (status) {
      case 'overdue': return '3px solid #DC2626';
      case 'in_progress': return '3px solid #2563EB';
      case 'completed': return '3px solid #16A34A';
      default: return 'none';
    }
  };
  ...
  const calendarEvents = events.map(event => ({
    ...
    backgroundColor: event.extendedProps.color || settings.colorScheme[event.source],
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Calendario HACCP</h1>
            <p className="text-gray-600">Gestione unificata di manutenzioni, controlli e attività</p>
          </div>
        </div>
      ...
      {showFilters && (
        <CollapsibleCard title="Filtri" isOpen={showFilters} onToggle={setShowFilters}>
          <CalendarFilter
            filter={filter}
      ...
      {showSettings && (
        <CollapsibleCard title="Impostazioni" isOpen={showSettings} onToggle={setShowSettings}>
          <CalendarSettings
            settings={settings}
