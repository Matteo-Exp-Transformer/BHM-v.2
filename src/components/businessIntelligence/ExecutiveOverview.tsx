  ...
  const loadExecutiveData = async () => {
    try {
      ...
      const [summaryData, kpisData, insightsData] = await Promise.all([
        executiveDashboard.getExecutiveSummary(timeRange.start, timeRange.end, companyIds),
    ...
    <div className="space-y-6">
      ...
      {insights.filter(insight => insight.priority === 'high').length > 0 && (
        <Alert variant="destructive">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            {insights.filter(insight => insight.priority === 'high').length} high-priority issues require executive attention
          </AlertDescription>
        </Alert>
      )}
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Portfolio Compliance</CardTitle>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {summary?.topPerformers.slice(0, 5).map((company, index) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className="font-bold">{company.overallScore.toFixed(1)}%</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {summary?.bottomPerformers.slice(0, 5).map((company, index) => (
                    <div 
                      key={company.companyId}
                      className="flex items-center justify-between p-2 hover:bg-muted rounded cursor-pointer"
                      onClick={() => onCompanyClick?.(company.companyId)}
                    >
                      <div className="flex items-center space-x-3">
                        <Badge 
                          variant={company.riskLevel === 'critical' ? 'destructive' : 'secondary'}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs text-muted-foreground">
                        <span>Trend: {kpi.trendValue > 0 ? '+' : ''}{kpi.trendValue.toFixed(1)}</span>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {summary?.topPerformers.concat(summary?.bottomPerformers || []).map((company) => (
                  <div 
                    key={company.companyId}
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className="text-center">
                        ...
                        <div className="font-bold text-lg">{company.ranking}</div>
                      </div>
                      <div>
                        <div className="font-semibold">{company.companyName}</div>
                        <div className="text-sm text-muted-foreground">
                          Last assessment: {company.lastAssessment.toLocaleDateString()}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-6">
                      <div className="text-center">
                        ...
                        <div className="font-bold text-lg">{company.overallScore.toFixed(1)}%</div>
                      </div>
                      <div className="text-center">
                        <div className="text-sm text-muted-foreground">Growth</div>
                        <div className={`font-bold text-lg ${company.growthRate > 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {company.growthRate > 0 ? '+' : ''}{company.growthRate.toFixed(1)}%
                        </div>
                      </div>
                      <Badge 
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <Badge variant={getInsightPriorityColor(insight.priority)}>
                          {insight.priority}
                        </Badge>
            ...
            <CardContent>
              <div className="space-y-4">
                {summary?.benchmarks.map((benchmark) => (
                  <div key={`${benchmark.industry}_${benchmark.metric}`} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-medium">{benchmark.metric}</h4>
                      <Badge variant={benchmark.percentile >= 75 ? 'default' : benchmark.percentile >= 50 ? 'secondary' : 'destructive'}>
                        {benchmark.percentile.toFixed(0)}th percentile
                      </Badge>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <span className="text-muted-foreground">Our Value:</span>
                        <div className="font-medium">{benchmark.ourValue.toFixed(1)}</div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Industry Avg:</span>
                        <div className="font-medium">{benchmark.average.toFixed(1)}</div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Top Quartile:</span>
                        <div className="font-medium">{benchmark.topQuartile.toFixed(1)}</div>
                      </div>
                      <div>
