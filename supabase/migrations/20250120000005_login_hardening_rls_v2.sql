-- Migration: Login Hardening RLS Policies v2
-- Description: Row Level Security policies for multi-tenant auth system with security hardening
-- Date: 2025-01-20
...
BEGIN
    current_user_uuid := auth.current_user_id();
    target_tenant_id := COALESCE(p_tenant_id, auth.current_tenant_id());
    ...
    RETURN EXISTS (
        ...
        JOIN roles r ON ur.role_id = r.id
        WHERE ur.user_id = current_user_uuid
        AND (target_tenant_id IS NULL OR ur.tenant_id = target_tenant_id)
        AND ur.is_active = TRUE
        AND (ur.expires_at IS NULL OR ur.expires_at > NOW())
        AND (
...
CREATE POLICY "Users can update own profile" ON users
    FOR UPDATE USING (
        ...
        id = auth.current_user_id()
        -- Additional checks for password changes handled in application logic
    );
...
-- Only system can create users directly (via service role)
...
-- Anonymous users can accept invites with valid token (limited update)
...
GROUP BY u.id, u.email, u.email_verified, u.created_at, u.last_login_at, u.status, u.profile, u.must_change_password;
...
-- Enable RLS on the view
ALTER VIEW active_sessions SET (security_invoker = true);

-- Grant permissions on the view
GRANT SELECT ON active_sessions TO authenticated;

-- Create view for pending invites (admin only)
...
AND auth.is_tenant_admin(i.tenant_id);

-- Enable RLS on the view
ALTER VIEW pending_invites SET (security_invoker = true);
...
COMMENT ON POLICY "System can create users" ON users IS 'Only system can create users directly';
