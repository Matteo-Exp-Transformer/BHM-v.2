...
describe('OnboardingWizard full flow', () => {
  ...
  it('completes full onboarding and redirects to dashboard', async () => {
    ...
    // Step 1: Business Info - Usa placeholder invece di label
    await waitFor(() => {
      const nameInput = screen.getByPlaceholderText(/nome della tua azienda/i)
      expect(nameInput).toBeInTheDocument()
    })

    // Compila Business Info usando placeholder
    fireEvent.change(screen.getByPlaceholderText(/nome della tua azienda/i), { target: { value: mockBusinessData.name } })
    fireEvent.change(screen.getByPlaceholderText(/indirizzo completo/i), { target: { value: mockBusinessData.address } })
    fireEvent.change(screen.getByPlaceholderText(/051 1234567/i), { target: { value: mockBusinessData.phone } })
    fireEvent.change(screen.getByPlaceholderText(/info@azienda.it/i), { target: { value: mockBusinessData.email } })
    fireEvent.change(screen.getByPlaceholderText(/IT12345678901/i), { target: { value: mockBusinessData.vat_number } })
    // Per select e date, usa selettori più specifici
    fireEvent.change(screen.getByDisplayValue(''), { target: { value: mockBusinessData.business_type } })
    fireEvent.change(screen.getByDisplayValue(''), { target: { value: mockBusinessData.established_date } })
    fireEvent.change(screen.getByPlaceholderText(/numero licenza/i), { target: { value: mockBusinessData.license_number } })
    ...
    // Aggiungi reparti - Usa placeholder o selettori più specifici
    const deptNameInput = screen.getByPlaceholderText(/nome reparto/i) || screen.getByDisplayValue('')
    const deptDescInput = screen.getByPlaceholderText(/descrizione/i) || screen.getByDisplayValue('')
    
    fireEvent.change(deptNameInput, { target: { value: mockDepartmentsData[0].name } })
    fireEvent.change(deptDescInput, { target: { value: mockDepartmentsData[0].description } })
    fireEvent.click(screen.getByText(/aggiungi/i))

    fireEvent.change(deptNameInput, { target: { value: mockDepartmentsData[1].name } })
    fireEvent.change(deptDescInput, { target: { value: mockDepartmentsData[1].description } })
    fireEvent.click(screen.getByText(/aggiungi/i))
  ...
  it('handles validation errors correctly', async () => {
    ...
    // Verifica che sia mostrato l'errore - Usa un approccio più flessibile
    await waitFor(() => {
      expect(screen.getByText(/completa tutti i campi/i)).toBeInTheDocument()
    })
  })
  ...
  it('loads saved data from localStorage on component mount', async () => {
    ...
    await waitFor(() => {
      // Usa selettori più flessibili per i dati caricati
      expect(screen.getByDisplayValue('Ristorante Test')).toBeInTheDocument()
      expect(screen.getByDisplayValue('Via Test 123')).toBeInTheDocument()
    })
  })
})
