...
test.describe('🎯 Test E - Allineamento Eventi Calendar ↔ Modal', () => {
  ...
  test('🔍 Verifica allineamento eventi tra calendario e modal', async ({ page }) => {
    console.log('🎯 Test E: Verifica allineamento eventi Calendar ↔ Modal');
    
    // Screenshot iniziale
    await page.screenshot({ path: 'test-e-alignment-iniziale.png', fullPage: true });
    ...
    await page.screenshot({ path: 'test-e-alignment-evento-calendario.png', fullPage: true });
    ...
    await page.screenshot({ path: 'test-e-alignment-dopo-click.png', fullPage: true });
    ...
    if (!modalVisible) {
      ...
      if (altModalVisible) {
        ...
        await page.screenshot({ path: 'test-e-alignment-modal-alternativo.png', fullPage: true });
      } else {
        console.log('❌ Nessun modal trovato');
        await page.screenshot({ path: 'test-e-alignment-no-modal.png', fullPage: true });
        return;
      }
    } else {
    ...
    // Screenshot finale
    await page.screenshot({ path: 'test-e-alignment-finale.png', fullPage: true });
    ...
    if (eventMatchesModal) {
      console.log('✅ Test E superato: Eventi calendar e modal allineati');
    } else {
      console.log('⚠️ Test E parziale: Modal aperto ma contenuto non perfettamente allineato');
    }

    console.log('🎯 Test E completato');
  });

  test('🔍 Verifica click su evento apre modal con eventi corretti del giorno', async ({ page }) => {
    console.log('🎯 Test E2: Verifica click evento apre modal con eventi corretti del giorno');
    
    // Screenshot iniziale
    await page.screenshot({ path: 'test-e2-click-giorno-iniziale.png', fullPage: true });
    ...
    // Screenshot dopo click
    await page.screenshot({ path: 'test-e2-click-giorno-dopo-click.png', fullPage: true });
    ...
    if (modalVisible) {
      ...
      // Screenshot finale
      await page.screenshot({ path: 'test-e2-click-giorno-finale.png', fullPage: true });
    } else {
      console.log('⚠️ Modal non aperto - verifico se ci sono altri elementi');
      await page.screenshot({ path: 'test-e2-click-giorno-no-modal.png', fullPage: true });
    }

    console.log('🎯 Test E2 completato');
  });
});
