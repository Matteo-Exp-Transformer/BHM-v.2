...
| 7 | **Scorte** (📦 inventario + scadenze + «cosa manca → lista spesa» in un tap §9.4; ciclo reinserimento dec.10; spunta ottimistica liste; filtro reparto) — [`07_SCORTE.html`](./MOCKUP_UI/07_SCORTE.html) | 🔄 **proposto**; copre la casa Scorte (unica senza mockup). *Onboarding dipendente: no mockup — stile admin semplificato, carta bianca a Fable (decisione owner).* |
