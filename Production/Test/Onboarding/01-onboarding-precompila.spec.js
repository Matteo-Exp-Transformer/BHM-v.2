...
// Fixtures
const { prefillBusinessData } = require('./fixtures/business-data')
...
const {
  ...
  getCurrentCompanyId,
  cleanupTestData,
} = require('./utils/supabase-assertions')

const { assertAllTabsShowData } = require('./utils/ui-assertions')
...
const {
  ...
  STEP_NAMES,
} = require('./utils/navigation-helpers')
...
test.describe('TEST 1: Onboarding con Precompila', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Compila form login
    await page.fill('input[name="email"], [data-testid="email-input"]', TEST_USER_EMAIL)
    await page.fill(
    ...
    // Attendi redirect (potrebbe andare a dashboard se già fatto onboarding, o a onboarding)
    await page.waitForLoadState('networkidle', { timeout: 10000 })

    const currentUrl = page.url()
    console.log('✅ Login completato, URL attuale:', currentUrl)
