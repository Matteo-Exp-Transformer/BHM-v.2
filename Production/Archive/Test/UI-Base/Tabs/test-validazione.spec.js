...
test.describe('Tabs.tsx - Test Validazione Dati', () => {
  ...
  test('Dovrebbe gestire Radix UI integration', async ({ page }) => {
    await page.goto('http://localhost:3000/dashboard');
    const radix = page.locator('[data-radix-tabs-root]');
  ...
  test('Dovrebbe gestire ForwardRef corretto', async ({ page }) => {
    await page.goto('http://localhost:3000/dashboard');
    const elements = page.locator('[role="tablist"], [role="tab"], [role="tabpanel"]');
  ...
  test('Dovrebbe gestire ClassName personalizzabile', async ({ page }) => {
    await page.goto('http://localhost:3000/dashboard');
    const styled = page.locator('.inline-flex, .rounded-md, .bg-muted');
